...
import com.hyphenate.easemob.easeui.EaseUI.EaseSettingsProvider;
...
//    }
    public synchronized void onNewMsg(EMMessage message, boolean showNotice) {
        boolean isSendNotification = isSendNotification(message);
        // check if app running background
        if (!EasyUtils.isAppRunningForeground(appContext)) {
            EMLog.d(TAG, "app is running in backgroud");
            sendNotification(message, false, isSendNotification);
        } else {
            if (showNotice) {
                sendNotification(message, true, isSendNotification);
            }
        }
    ...
    private boolean isSendNotification(List<EMMessage> message) {
        if (message == null || message.isEmpty()) {
            return false;
        }
        return isSendNotification(message.get(message.size() - 1));
    }

    private boolean isSendNotification(EMMessage message) {
    ...
    public synchronized void onNewMesg(List<EMMessage> messages) {
        boolean isSendNotificaton = isSendNotification(messages);
        // check if app running background
        if (!EasyUtils.isAppRunningForeground(appContext)) {
            EMLog.d(TAG, "app is running in backgroud");
            sendNotification(messages, false, isSendNotificaton);
        } else {
            sendNotification(messages, true, isSendNotificaton);
        }
     ...
     * @param isForeground
     */
    protected void sendNotification(List<EMMessage> messages, boolean isForeground, boolean isSendNotification) {
        ...
        sendNotification(messages.get(messages.size() - 1), isForeground, false, isSendNotification);
    }

    protected void sendNotification(EMMessage message, boolean isForeground, boolean isSendNotification) {
        sendNotification(message, isForeground, true, isSendNotification);
    }
     ...
     * @param message
     */
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease, boolean isSendNotification) {
...
//            String notifyText = username + " ";
            ...
            if (totalCount > 0) {
                ...
                try {
                    BadgeUtil.sendBadgeNotification(notification, 0, appContext, totalCount, totalCount);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
                try {
                    BadgeUtil.resetBadgeCount(appContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
...
//                notificationManager.cancel(foregroundNotifyID);
    ...
    public void vibrateAndPlayTone(EMMessage message) {
        this.vibrateAndPlayTone(message, true);
    }

    /**
     * vibrate and  play tone
     */
    public void vibrateAndPlayTone(EMMessage message, boolean playRing) {
        if (message != null) {
            if (message.getFrom().equals(EMClient.getInstance().getCurrentUser())) {
                return;
            }
