    ...
    public boolean containsAtUsername(String content){
        ...
        synchronized (toAtUserList) {
            for(String username : toAtUserList){
                ...
                if(content.contains(nick) || content.contains(atAll)){
                    return true;
                }
            }
        }
            ...
            if(msg.getChatType() == ChatType.GroupChat){
                ...
                try {
                    ...
                    for(int i = 0; i < jsonArray.length(); i++){
                        ...
                        String children = jsonObj.optString("children");
                        JSONObject jsonData = jsonObj.optJSONObject("data");
                        if (jsonData == null) {
                            continue;
                        }
                        String imUsername = jsonData.optString("imUsername");
                        if(EMClient.getInstance().getCurrentUser().equals(imUsername)){
                            if(!atMeGroupList.contains(groupId)){
                                atMeGroupList.add(groupId);
                                break;
                            }
                        } else if (EaseConstant.MESSAGE_ATTR_AT_ALL_TYPE.equals(jsonData.optString("type"))) {
                            if(!atMeGroupList.contains(groupId)){
                                atMeGroupList.add(groupId);
                                break;
                            }
