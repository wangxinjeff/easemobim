...
public class GroupSearchMessageActivity extends BaseActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                intent.putExtra(EaseConstant.EXTRA_SELECTED_MSGID, message.getMsgId());
    ...
    private void searchMessages(){
        ...
        EaseUI.getInstance().execute(new Runnable() {
            public void run() {
                String searchText = query.getText().toString();
                EMConversation conversation = EMClient.getInstance().chatManager().getConversation(groupId);
                List<EMMessage> resultList = conversation.searchMsgFromDB(searchText, System.currentTimeMillis(), 500, null, EMSearchDirection.UP);
                messageList.clear();
