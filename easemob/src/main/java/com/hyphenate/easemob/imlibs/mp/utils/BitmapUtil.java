...
public class BitmapUtil {
	...
	public static int getScreenWidthPix(){
		if (s_ScreenWidth == 0){
			s_ScreenWidth = MPClient.get().getAppContext().getResources().getDisplayMetrics().widthPixels;
		}
	...
	public static int getScreenHeightPix(){
		if (s_ScreenHeight == 0){
			s_ScreenHeight = MPClient.get().getAppContext().getResources().getDisplayMetrics().heightPixels;
		}
	...
	public static boolean saveBitmap(String bitName, Bitmap bitmap, String suffix) {
		try {
			...
			if (suffix.equals(Bitmap.CompressFormat.PNG)){
	...
	public static boolean compressBmpToFile(Bitmap bmp, File file, int sizeKB, String suffix){
		...
		int compressCount = 0;
		boolean isJpg = TextUtils.equals(suffix, ".jpg");
		...
		MPLog.d(TAG, "执行第1次压缩，压缩前质量>>" + sizeInKb + "kb");
		while(sizeInKb > sizeKB && (options - 5) > 0){
			...
			MPLog.d(TAG, "第" + compressCount + "次压缩完成，压缩后质量 >> " + sizeInKb + "kb");
		}
		try {
