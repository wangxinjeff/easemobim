package com.hyphenate.easemob.imlibs.officeautomation.emrequest.okhttp;
...
public class PersistentCookieStore implements CookieStore {
	...
	public PersistentCookieStore(Context context) {
		...
		Map<String, ?> prefMap = cookiePrefs.getAll();
			...
			if (entry.getValue() != null && !((String) entry.getValue()).startsWith(COOKIE_NAME_PREFIX)) {
				...
				for (String name : cookieNames) {
					...
					if (encodedCookie != null) {
						HttpCookie decodedCookie = decodedCookie(encodedCookie);
	...
	public void add(URI uri, HttpCookie cookie) {
		...
		prefsWriter.putString(COOKIE_NAME_PREFIX + name, encodedCookie(new SerializableHttpCookie(cookie)));
		prefsWriter.apply();
	}
	 ...
	 */
	protected String encodedCookie(SerializableHttpCookie cookie) {
		if (cookie == null)
			return null;
		...
		try {
			...
			Log.d(LOG_TAG, "IOExecption in encodeCookie", e);
			return null;
		}
	 ...
	 * @return
	 */
	protected HttpCookie decodedCookie(String cookieString) {
	...
	/**
	 * Using some super basic byte array &lt;-&gt; hex conversations so we don't
	 * have to rely on any large Base64 libraries. Can be overriden if you like!
	 * 
	 * @param bytes
	 * @return
	 */
